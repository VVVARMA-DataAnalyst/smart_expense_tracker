...
Return only the category name that best matches this transaction.`;
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "You are a financial categorization assistant. Return only the category name, nothing else." },
    ...
    console.log(`Categorized transaction: ${merchant} -> ${category}`);

    return new Response(
      JSON.stringify({ category, confidence: 0.85 }),
